...
async def scan_page(
    ...
    """
    Scan a single page of licenses with backoff retry logic
    """
    params = {
        "name": name,
        "value": value,
    ...
    while True:
        try:
            response = requests.get(f"{base_url}/licenses", headers=auth_headers, params=params)
...
async def scan_key_range(
    ...
    """
    Scan licenses within a specific key range
    """
    results = []
...
def main(
    ...
    Example:
    $ python scan_licenses.py --name "status" --value "\"active\"" --output results.csv
    """
    ...
    click.echo(f"Starting parallel license scan with {partitions} partitions")

    # Ensure value is properly formatted JSON
    try:
    ...
    # Display summary
    click.echo(f"\nFound {len(results)} matching licenses")
